    ...
    <>
      <header className={`header`}>
        <div className="header__container">
          ...
          <button onClick={()=>{setIsNavVisible(current => !current)}} className="header__mobile-nav">
            <img className="header__hamburger" src={!isNavVisible ? hamburger : closeNav} alt="hamburger menu" />
          </button>
        </div>
      </header>
      {isNavVisible && <MainNav isLoggedIn={isLoggedIn} setIsLoggedIn={setIsLoggedIn} /> }
    </>
  )
}
